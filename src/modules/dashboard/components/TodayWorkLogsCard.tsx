...
export function TodayWorkLogsCard({ orgSlug, memberId }: Readonly<TodayWorkLogsCardProps>) {
  ...
  const { data: projects = [], isLoading: isLoadingProjects } = useProjectsForAttendance(orgSlug, memberId);
  ...
  const cardClasses = "bg-surface rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col";

  if (isLoading || isLoadingProjects) {
    return (
      <div className={cardClasses}>
        <div className="px-6 py-5 border-b border-black/[0.04]">
          <h2 className="text-[17px] font-semibold text-neutral-900 tracking-tight">Today&apos;s Work Logs</h2>
        </div>
        ...
        <div className="p-6 flex-1">
          {logs.length === 0 ? (
            <button
              type="button"
              onClick={handleOpenCreate}
              className="flex w-full flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-200 bg-canvas py-12 text-center transition-colors hover:bg-neutral-50"
            >
              ...
              <p className="text-sm font-semibold text-neutral-900">No work logs yet</p>
              <p className="mt-1 text-sm text-neutral-500">Add your first log to track today&apos;s work</p>
            </button>
          ) : (
            <div className="flex flex-col">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="flex items-start justify-between gap-4 border-b border-black/4 py-4 last:border-b-0"
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-neutral-900" title={getLogTitle(log)}>
                      {getLogTitle(log)}
                    </p>
                    ...
                    {log.notes && (
                      <p className="mt-2 line-clamp-2 text-[13px] text-neutral-500" title={log.notes}>
                        {log.notes}
                      </p>
                    )}
                  </div>
